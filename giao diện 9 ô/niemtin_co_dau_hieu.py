...
def dfs(start_states, goal_states, hint):
    """Thuật toán DFS để tìm tất cả các trạng thái từ trạng thái ban đầu đến các trạng thái đích."""
    visited = set()
    visited_states = []  # Lưu tất cả các trạng thái đã xét
    paths = []  # Lưu tất cả các đường đi từ trạng thái đầu đến trạng thái đích

    def dfs_recursive(state, path):
        flat_state = flatten(state)
        if flat_state in visited:
            return False
        visited.add(flat_state)
        ...
        # Sinh các trạng thái lân cận và tiếp tục tìm kiếm
        for neighbor in get_neighbors_with_hint(state, hint):
            if flatten(neighbor) not in visited:
                if dfs_recursive(neighbor, path + [neighbor]):
                    return True
        return False

    for start_state in start_states:
        dfs_recursive(start_state, [start_state])
...
def a_star(start_states, goal_states, hint):
    """Thuật toán A* để tìm tất cả các trạng thái từ trạng thái ban đầu đến các trạng thái đích."""
    visited = set()
    visited_states = []  # Lưu tất cả các trạng thái đã xét
    paths = []  # Lưu tất cả các đường đi từ trạng thái đầu đến trạng thái đích
    ...
    for start_state in start_states:
        for goal_state in goal_states:
            queue = [(0 + heuristic(start_state, goal_state), 0, start_state, [start_state])]  # (f, g, state, path)

            while queue:
                queue.sort()  # Sắp xếp hàng đợi theo f(n)
                _, g, state, path = queue.pop(0)
                flat_state = flatten(state)
                ...
                for neighbor in get_neighbors_with_hint(state, hint):
                    ...
                    if flat_neighbor not in visited:
                        ...
                        queue.append((g + 1 + h, g + 1, neighbor, path + [neighbor]))
...
def ida_star(start_states, goal_states, hint, max_nodes=100000):
    """Thuật toán IDA* tối ưu cho 8-puzzle. Mỗi trạng thái đầu chỉ tìm 1 đường đi đến 1 goal."""
    visited_states = []
    paths = []
    node_count = [0]
...
def greedy_search(start_states, goal_states, hint):
    ...
    for start_state in start_states:
        found = False
        for goal_state in goal_states:
            priority_queue = [(heuristic(start_state, goal_state), start_state, [start_state])]  # (h, state, path)
            visited = set()

            while priority_queue:
                priority_queue.sort(key=lambda x: x[0])  # Sắp xếp hàng đợi theo giá trị heuristic
...
def and_or_search(start_states, goal_states, hint, max_depth=30):
    """Thuật toán AND-OR Search cho 8-puzzle với dấu hiệu."""
    visited_states = []
    paths = []
...
def display_logs3():
    global entries
    root = tk.Tk()
    root.title("Niềm tin có dấu hiệu")
    ...
    max_col = 7
        ...
        # Tạo frame chứa các ô nhập liệu cho dấu hiệu
        # Tạo frame chứa các ô nhập liệu cho dấu hiệu (giữa giao diện)
    entry_frame = tk.Frame(root, bg="lightblue", width=180, height=180)
    entry_frame.grid(row=0, column=0, columnspan=max_col)
    entry_frame.grid_propagate(False)  # Không cho frame tự co giãn

    entries = [[tk.Entry(entry_frame, width=2, font=("Arial", 28), bg="lightyellow", justify="center") for _ in range(3)] for _ in range(3)]
    for i in range(3):
        for j in range(3):
            entries[i][j].grid(row=i, column=j, padx=10, pady=10, sticky="nsew")
            entry_frame.grid_columnconfigure(j, weight=1, minsize=60)
        entry_frame.grid_rowconfigure(i, weight=1, minsize=60)
